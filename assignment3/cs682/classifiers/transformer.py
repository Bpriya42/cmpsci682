...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        memory = self.visual_projection(features)  # Shape: (N, wordvec_dim)

        # Step 2: Embed captions and add positional encoding
        caption_embeddings = self.embedding(captions)  # Shape: (N, T, wordvec_dim)
        caption_embeddings = self.positional_encoding(caption_embeddings)  # Shape: (N, T, wordvec_dim)

        # Step 3: Create target mask to prevent attending to future positions
        T = captions.size(1)
        tgt_mask = torch.tril(torch.ones(T, T, device=captions.device)).bool()

        # Step 4: Pass through Transformer Decoder
        transformer_output = self.transformer(tgt=caption_embeddings, memory=memory.unsqueeze(1), tgt_mask=tgt_mask)

        # Step 5: Project decoder outputs to vocabulary
