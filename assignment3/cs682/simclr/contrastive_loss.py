...
def sim(z_i, z_j):
    ...
    if z_i.ndim == 1 and z_j.ndim == 1:  # Both are vectors
        norm_dot_product = torch.dot(z_i, z_j) / (torch.linalg.norm(z_i) * torch.linalg.norm(z_j))
    else:  # Batchwise computation
        norm_dot_product = torch.mm(z_i, z_j.T) / (
            torch.linalg.norm(z_i, dim=1, keepdim=True) * torch.linalg.norm(z_j, dim=1, keepdim=True).T
        )
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_pos = sim(z_k, z_k_N)
        
        # All pairs similarity for z_k
        sim_all_zk = torch.exp(sim(out[k].unsqueeze(0), out) / tau).squeeze()
        denom_k = sim_all_zk.sum() - torch.exp(sim_pos / tau)

        # Loss for positive pairs
        l_k_kN = -torch.log(torch.exp(sim_pos / tau) / denom_k)
        l_kN_k = -torch.log(torch.exp(sim_pos / tau) / denom_k)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # Normalize the vectors in out_left and out_right
    ...
    # Compute dot products for positive pairs
...
def compute_sim_matrix(out):
    ...
    # Normalize the rows of the input tensor
    out_norm = out / torch.linalg.norm(out, dim=1, keepdim=True)
    
    # Compute the dot product for all pairs using matrix multiplication
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pos = torch.diag(sim_matrix[:N, N:]).reshape(-1, 1)  # Extract the diagonal elements for positive pairs: [N]
    sim_pos = sim_pos.repeat(2, 1).to(device)  
    ...
    numerator = torch.exp(sim_pos / tau)  # [N, 1]
